...
#         self._bottom_rect = [QPoint(x, y) for x in range(1, self.width() - self._padding) for y in range(self.height() - self._padding, self.height() + 1)]
...
class LoadingGifDemo(QWidget):
    ...
    def initUI(self):
        # 设置定位和左上角坐标
        self.setGeometry(300, 500, 360, 360)
        # 设置窗口标题
        ...
        self.movie = QMovie('./gif/keli.gif')
